...
vals = {s=>{n=>[r(y[1]) for y in filter(x->l(x[1])==n && x[2]==s, nums)]
            for n in unique([l(x[1]) for x in filter(x->x[2]==s, nums)])} 
        for s=3:8}
...
function findit(vals, seq, i, k, path=Any[])
    if i > length(seq)
        if path[1][2] == k
            println(path)
            println(sum(map(x->x[2]*100+x[3],path)))
            return path
        else
            return
        end
...
found = false
for seq in collect(permutations(3:8))
    # seq = [3,5,4]

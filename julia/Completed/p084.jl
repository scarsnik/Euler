...
function diceroll(sides::Int)
	d1 = rand(1:sides)
	d2 = rand(1:sides)
	isdouble = d1==d2 ? true : false
	return d1+d2, isdouble
end

function move(square::Int, moves::Int, chCards, ccCards)
	boardsize = 40
	square += moves
	if square > boardsize
		r = square % boardsize
		square = r == 0 ? boardsize : r
	end
	...
	# Check Chance Card
	if square in [8, 23, 37]
		square = chCard(square, chCards)
	end

	# Check Community Chest Card
	if square in [3, 18, 34]
		square = ccCard(square, ccCards)
	end

	return square
end

function chCard(square, cards)
	# CH1 = 8
	# CH2 = 23
	# CH3 = 37
	card = shift!(cards)
	...
	card == 1 && return 1            # Advance to GO
	card == 2 && return 11           # Go to JAIL
	card == 3 && return 12           # Go to C1
	card == 4 && return 25           # Go to E3
	card == 5 && return 40           # Go to H2
	card == 6 && return 6            # Go to R1

	card in [7,8] && square == 37 && return 6  # Go to R1
	card in [7,8] && square == 8  && return 16 # Go to R2
	card in [7,8] && square == 23 && return 26 # Go to R3

	card == 9 && square in [8,37] && return 13 # Go to U1
	card == 9 && square == 23     && return 29 # Go to U2

	card == 10 && return square - 3  # Go back 3 squares
	return square
end

function ccCard(square, cards)
	# CC1 = 3
	# CC2 = 18
	# CC3 = 34
	card = shift!(cards)
...
square = 1
for rolls = 1:10^6
	moves, isdouble = diceroll(sides)
	doubles += isdouble ? 1 : 0
	if doubles == 3
		square = 11
		doubles = 0
	else
		square = move(square, moves, CH, CC)
		# if dice roll is not a double then reset double count
		if !isdouble 
			doubles = 0
		end
	end
	landings[squares[square]] += 1
end

top3 = sort(collect(landings), by=f->f[2], rev=true)[1:3]
answer = join(map(string, find(f->f in map(f->f[1], top3), squares) - 1))
println("Problem 84: $answer")
toc()
